...
router.post('/', async function (req, res, next) {
    ...
    client.messages.create({
        body: req.body.organizationName + ': ' + code + ' - This is your verification code.',
        to: '+' + req.body.countryCode + req.body.mobile,  // Text this number
        from: '+14048003419' // From a valid Twilio number
    }).then((message) => {
        User.update({ authCode: code }, { where: { email: req.body.email.toLowerCase(), organizationId: req.body.organizationId } }).then(() => {
...
router.post('/verify-sms', passport.authenticate('jwt', { session: false }), async function (req, res, next) {
    ...
    User.findOne({ where: { id: req.user.id, organizationId: req.user.orgId } }).then((data) => {
        let date = new Date();
